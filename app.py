...
# ---- TOP KPI'S ----
total_produccion = int(df_selection["ASIGNADO $"].sum())
produccion_ito_enel = int(df_selection["ASIGNADO $"].sum())
...
with right_column:
    st.subheader("% Avance Meta Mensual")
    st.subheader(f"% {(100 * total_produccion)/200000000}")


st.markdown("----")

total_by_ito=(
    df_selection.groupby(by=["INSPECTOR"]).sum()[["ASIGNADO $"]].sort_values(by="ASIGNADO $"))
